...
public class AdminLoginService {
	...
	public LoginResponse login(LoginRequest loginRequest) throws JOSEException, ParseException {
		...
		var user = adminRepository.findByUsername(loginRequest.getUsername())
		...
		if (!password) {
			throw new HandleRuntimeException(ErrorCode.ADMIN_LOGIN_FAILED);
		}
	...
	private String generateToken(Admin admin) {
		...
		JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder().subject(admin.getUsername())
				.issuer("demoLoginSSport.com").issueTime(new Date())
				.expirationTime(new Date(Instant.now().plus(VALID_DURATION, ChronoUnit.SECONDS).toEpochMilli()))
				.jwtID(UUID.randomUUID().toString()) // Tạo ID cho token
				.claim("scope", "ROLE_" + admin.getRole()).build();
	...
	public LoginResponse refreshNewToken(RefreshTokenRequest refreshTokenRequest) throws JOSEException, ParseException {
		...
		if ("ROLE_ADMIN".equals(scope)) {
			...
			var tokenRefresh = generateToken(admin);
